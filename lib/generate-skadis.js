 ...
 * @returns {string}
 */
export function generateSkadis({ rows, columns, fill = 'uneven', shape = 'pill' }) {
	if (
	...
	for (let row = 0; row < rows; row++) {
		for (let column = 0; column < columns; column++) {
			...
			if (fill === 'full' || even == (fill == 'even')) {
				...
				const shapeFn =
					shape === 'pill' ? pill_slot :
					shape === 'rectangle' ? rectangle_slot :
					unreachable(shape);

				slots.push(shapeFn(x, y));
			}
		}
	}
